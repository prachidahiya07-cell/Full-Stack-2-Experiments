    ...
    <main className="workspace">
      ...
      <section className="content">
        ...
        <div className="main-grid">
          ...
          <aside className="render-lab"><div className="lab-head"><div><span className="eyebrow">RENDER LAB</span><h2>ON</h2></div><button aria-label="More options">•••</button></div><div className="mode-switch"><button className={!optimized ? 'is-active' : ''} onClick={() => setOptimized(false)}>Non-optimized</button><button className={optimized ? 'is-active' : ''} onClick={() => setOptimized(true)}>Optimized</button></div><button className="reset-button" onClick={() => performanceMonitor.reset()}>Reset count</button><div className="render-total"><strong>{renderCount}</strong><span>app renders</span></div><div className="calculation"><h3>Rendering Calculation</h3><div><span>Calendar posts</span><strong>{monthPosts.length}</strong></div><div><span>Optimization</span><strong>{optimized ? 'Optimized' : 'Not Optimized'}</strong></div><div><span>Render cycles</span><strong>{renderCount}</strong></div><div className="meter"><span style={{ width: `${efficiency}%` }} /></div><small>Render efficiency: {efficiency}%</small></div><div className={`lab-note ${optimized ? 'lab-note--blue' : ''}`}><span>●</span><div><strong>{optimized ? 'Optimized rendering' : 'Non-optimized rendering'}</strong><small>{optimized ? 'Only changed items update.' : `Each drag updates all ${calendarDays} days.`}</small></div></div>{selectedPosts.length > 0 && <div className="selected-post"><span className="eyebrow">SELECTED DATE</span><strong>{format(selectedDate, 'MMM d')}</strong><span>{selectedPosts[0].title}</span></div>}</aside>
        </div>
      </section>
    </main>
  );
